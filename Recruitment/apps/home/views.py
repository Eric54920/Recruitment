...
class CompanyListAPIView(ListAPIView):
    """查看所有公司列表"""
    queryset = models.Company.objects.filter(is_show=True, is_delete=False, is_pass=True).order_by('orders')
...
class CompanyDetailAPIView(mixins.RetrieveModelMixin,
    ...
    queryset = models.Company.objects.filter(is_show=True, is_delete=False, is_pass=True)
    serializer_class = serializers.CompanyDetailSerializer

class CompanyRetrieveAPIView(mixins.RetrieveModelMixin,
...
def search_company(request):
    ...
    data = models.Company.objects.filter(companyFullName__icontains=queryString, is_show=True, is_delete=False, is_pass=True).values('companyFullName')
    lst = []
    for i in data:
