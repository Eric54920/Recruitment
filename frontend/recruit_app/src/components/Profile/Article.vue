  ...
  <div class="article">
    <div class="container">
      <div class="article-content">
        <div class="header">
          <span class="avatar">
            <img class="img" :src="article.userinfo.avatar" />
          </span>
          <div class="user-info">
          ...
          <el-dropdown class="more-operate" trigger="hover" v-if="this.user_id == article.userinfo.id">
        ...
        <div class="article-header">
          ...
          <p class="tags">
            <span v-for="(item, i) in article.tags" :key="i">{{ item }}</span>
          </p>
        </div>
        ...
        <div class="footer">
          ...
          <span @click="like(article.id)">
            <i class="fa fa-thumbs-up"></i> 赞 {{ article.like }}
          </span>
        </div>
      </div>
      <div class="comment">
        ...
        <div class="comment-item" v-for="(comment, i) in comments_list" :key="i">
            <el-avatar :size="size" :src="comment.user.avatar"></el-avatar>
            <div class="comment-content">
                <div class="detail">
                    <p class="text"><a :href="`/user/${comment.user.uid}`" class="from-name">{{ comment.user.nic_name }}</a>&nbsp;:&nbsp;{{ comment.content }}</p>
                    <div class="status">
                ...
                <div class="sub" v-show="comment.sub_comment">
                    <div class="detail" v-for="(sub, i) in comment.sub_comment" :key="i">
                        <p class="text">
                          <a :href="`/user/${sub.user.uid}`" class="from-name">{{ sub.user.nic_name }} <i v-show="article.userinfo.id == sub.user.uid">作者</i></a> @ 
                          <a :href="`/user/${sub.user.uid}`" class="from-name">{{ sub.reply.nic_name }} <i v-show="article.userinfo.id == sub.reply.uid">作者</i></a>&nbsp;:&nbsp;{{ sub.content }}
                        </p>
                        <div class="status">
